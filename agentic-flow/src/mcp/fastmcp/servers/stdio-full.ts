...
// ADR-0288: the in-process agentdb service island (service class + wrapping
// tool modules) was retired; this server registers only island-free modules.
// The canonical agent-facing agentdb MCP surface is the standalone `agentdb`
// package / the ruflo aggregator (`mcp__ruflo__agentdb_*`).
...
console.error('Loading tools: core (11), github (8), ruvector (6), sona-rvf (11), infrastructure (13), autopilot (10), cost-optimizer (4), streaming (8), sona-rl (8), quantization (8), explainability (10), enhanced-booster (9) — agentdb-wrapping tools retired per ADR-0288');
...
// Register modular tool sets (island-free modules only — ADR-0288)
registerGitHubTools(server);          // 8 tools
registerRuVectorTools(server);        // 6 tools
registerSonaRvfTools(server);         // 11 tools
registerInfrastructureTools(server);  // 13 tools
registerAutopilotTools(server);       // 10 tools
registerCostOptimizerTools(server);     // 4 tools (Cost optimizer - ADR-064)
registerStreamingTools(server);         // 8 tools (Streaming architecture - ADR-065 P1-3; search-streaming retired per ADR-0288)
...
console.error('Registered island-free tool set (ADR-0288 slim)');
